...
use wgpu::{
    Adapter, Device, DeviceType, PollStatus, Queue, StoreOp, Surface, SurfaceConfiguration, TextureFormat
};
...
impl VgerRenderer {
            ...
            BrushRef::Gradient(g) => match g.kind {
                GradientKind::Linear(linear) => {
                    ...
                    let start = floem_vger_rs::defs::LocalPoint::new(
                        linear.start.x as f32 * first_stop.offset,
                        linear.start.y as f32 * first_stop.offset,
                    );
                    let end = floem_vger_rs::defs::LocalPoint::new(
                        linear.end.x as f32 * second_stop.offset,
                        linear.end.y as f32 * second_stop.offset,
                    );
                    self.vger
    ...
    fn render_image(&mut self) -> Option<peniko::ImageBrush> {
        ...
        loop {
            ...
            if let Ok(PollStatus::WaitSucceeded) = self.device.poll(wgpu::PollType::Wait) {
                rx.recv().ok()?.ok()?;
                break;
            }
        }
